...
pub struct Bls12381AggregatePopVerifiedPubkeysGasParameters {
    ...
    pub per_pubkey_cost: InternalGas,
}
...
fn native_bls12381_aggregate_pop_verified_pubkeys(
    ...
    let per_pubkey_cost: u64 = gas_params.per_pubkey_cost.into();
    let cost = base_cost + per_pubkey_cost * pks_serialized.len() as u64;
    ...
    for pk_bytes in pks_serialized {
        ...
        let pk = match bls12381::PublicKey::try_from(&pk_bytes[..]) {
            ...
            Err(_) => return Ok(NativeResult::ok(cost.into(), smallvec![none_option()])),
        };
    ...
    if pks.is_empty() {
        return Ok(NativeResult::ok(cost.into(), smallvec![none_option()]));
    }
...
fn native_bls12381_verify_signature(
    ...
    cost += unit_cost * msg_bytes.len() as u64;
