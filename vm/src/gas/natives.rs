...
macro_rules! expand_get_for_native_gas_params {
    ...
    ($(.$field: ident)+, $(optional $($dummy: ident)?)? $key: literal, $initial_val: expr, $param_ty: ty, $package_name: literal, $params: ident, $gas_schedule: ident) => {
        $crate::gas::natives::expand_get_impl_for_native_gas_params!($params $(.$field)+, $gas_schedule, $package_name, $(optional $($dummy)?)? $key);
    }
}
...
macro_rules! define_gas_parameters_for_natives {
    ($param_ty: ty, $package_name: literal, [$([$($t: tt)*]),* $(,)?] $(, allow_unmapped = $allow_unmapped: expr)?) => {
        impl crate::gas::gas_meter::FromOnChainGasSchedule for $param_ty {
            fn from_on_chain_gas_schedule(gas_schedule: &std::collections::BTreeMap<String, u64>) -> Option<Self> {
                let mut params = <$param_ty>::zeros();

                $(
                    crate::gas::natives::expand_get_for_native_gas_params!($($t)*, $param_ty, $package_name, params, gas_schedule);
                )*
        ...
        impl crate::gas::gas_meter::ToOnChainGasSchedule for $param_ty {
            fn to_on_chain_gas_schedule(&self) -> Vec<(String, u64)> {
                [$(crate::gas::natives::expand_kv_for_native_gas_params!($($t)*, self)),*]
                    .into_iter().map(|(key, val)| (format!("{}.{}", $package_name, key), val)).collect()
            }
        }

        impl crate::gas::gas_meter::InitialGasSchedule for $param_ty {
            fn initial() -> Self {
                let mut params = <$param_ty>::zeros();

                $(
                    crate::gas::natives::expand_set_for_native_gas_params!($($t)*, $param_ty, $package_name, params);
                )*
        ...
        fn keys_should_be_unique() {
            ...
            for key in [$(crate::gas::natives::extract_key_for_native_gas_params!($($t)*)),*] {
        ...
        fn paths_must_be_unique() {
            ...
            for path in [$(crate::gas::natives::extract_path_for_native_gas_params!($($t)*)),*] {
        ...
        fn all_parameters_mapped() {
            ...
            let mapped = [$(crate::gas::natives::extract_key_for_native_gas_params!($($t)*)),*].len() $(+ $allow_unmapped)?;
            if mapped != total {
...
mod tests {
    use super::*;
    use crate::gas::gas_meter::FromOnChainGasSchedule;
