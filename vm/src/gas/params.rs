...
macro_rules! define_gas_parameters {
    (
        ...
        impl $crate::gas::gas_meter::FromOnChainGasSchedule for $params_name {
            fn from_on_chain_gas_schedule(gas_schedule: &std::collections::BTreeMap<String, u64>) -> Option<Self> {
                let mut params = $params_name::zeros();

                $(
                    $crate::gas::params::expand_get_for_gas_parameters!(params . $name, gas_schedule, $prefix, $(optional $($dummy)?)? $key);
                )*
        ...
        impl $crate::gas::gas_meter::ToOnChainGasSchedule for $params_name {
        ...
        impl $crate::gas::gas_meter::InitialGasSchedule for $params_name {
            fn initial() -> Self {
                Self {
                    $($name: $initial.into()),*
                }
