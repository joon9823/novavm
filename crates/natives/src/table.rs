...
pub struct TableChangeSet {
    ...
    pub payer_changes: BTreeMap<TableHandle, AccountAddress>,
}
...
struct TableData {
    ...
    payer_changes: BTreeMap<TableHandle, AccountAddress>,
}
...
pub fn all_natives(table_addr: AccountAddress, gas_params: GasParameters) -> NativeFunctionTable {
    let natives: [(&str, &str, NativeFunction); 9] = [
        ...
        (
            "table",
            "set_table_payer",
            make_native_set_table_payer(gas_params.set_table_payer),
        ),
        (
            "table",
            "add_box",
...
fn native_new_table_handle(
    ...
    let payer = pop_arg!(args, AccountAddress);
    assert!(table_data
        .payer_changes
        .insert(TableHandle(handle), payer)
        .is_none());
...
fn native_set_table_payer(
    ...
    let table_context = context.extensions().get::<NativeTableContext>();
    let mut table_data = table_context.table_data.borrow_mut();
    let _ = table_data.payer_changes.insert(handle, payer);

    Ok(NativeResult::ok(gas_params.base, smallvec![]))
}

pub fn make_native_set_table_payer(gas_params: SetTablePayerGasParameters) -> NativeFunction {
    Arc::new(
        move |context, ty_args, args| -> PartialVMResult<NativeResult> {
