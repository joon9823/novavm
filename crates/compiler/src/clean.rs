...
fn validate_manifest(base_path: &Path) -> anyhow::Result<()> {
        ...
        let package_name = manifest.get("package").unwrap().get("name").unwrap().as_str().unwrap();
        ...
        if !(build_path.join(&package_name).is_dir()) {
            bail!("built package {} not found in {}", package_name, build_path.display())
        }
...
fn remove(path_to_clean: PathBuf) -> anyhow::Result<()> {
    let remove = if path_to_clean.is_dir() { std::fs::remove_dir_all } else { std::fs::remove_file };
