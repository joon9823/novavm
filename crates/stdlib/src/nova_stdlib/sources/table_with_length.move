...
module nova_std::table_with_length {
    use std::error;
    ...
    /// Create a new Table.
    public fun new<K: copy + drop, V: store>(payer: &signer): TableWithLength<K, V> {
        TableWithLength {
            inner: table::new<K, V>(payer),
            length: 0,
        }
    }

    /// Set storage fee payer for a table.
    public fun set_payer<K: copy + drop, V: store>(table: &TableWithLength<K, V>, payer: &signer) {
        table::set_payer<K, V>(&table.inner, payer)
    }
