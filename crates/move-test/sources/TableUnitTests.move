module TestAccount::TableUnitTests {
    ...
    #[test(s = @0x42)]
    fun simple_read_write(s: signer) {
    ...
    #[test(s = @0x42)]
    #[expected_failure(abort_code = 196710)]
    fun test_destroy_fails(s: signer) {
        let t = T::new<u64, u64>(&s);
        T::add(&mut t, 1, 2);
        assert!(*T::borrow(&t, 1) == 2, 1);
    ...
    fun test_vector(s: signer) acquires S {
        let t = T::new<u8, vector<address>>(&s);
    ...
    fun test_table_of_tables(s: signer) {
        let t = T::new<address, T::Table<address, u128>>(&s);
        let val_1 = 11;
        let val_2 = 45;
        let val_3 = 78;

        // Create two small tables
        let t1 = T::new<address, u128>(&s);
        T::add(&mut t1, @0xAB, val_1);

        let t2 = T::new<address, u128>(&s);
    ...
    #[test(s = @0x42)]
    fun test_add_after_remove(s: signer) {
        let t = T::new<u64, u64>(&s);
        T::add(&mut t, 42, 42);
    ...
    #[test(s = @0x42)]
    #[expected_failure(abort_code = 25863)]
    fun test_remove_removed(s: signer) {
        let t = T::new<u64, u64>(&s);
        T::add(&mut t, 42, 42);
        let forty_two = T::remove(&mut t, 42);
        assert!(forty_two == 42, 101);
    ...
    #[test(s = @0x42)]
    #[expected_failure(abort_code = 25863)]
    fun test_borrow_removed(s: signer) {
        let t = T::new<u64, u64>(&s);
        T::add(&mut t, 42, 42);
        let forty_two = T::remove(&mut t, 42);
        assert!(forty_two == 42, 101);
